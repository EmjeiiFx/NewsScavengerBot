...
def generate_us30_signal(
    ...
    if price > ema200 and ema20 > ema50 and rsi14 > 55 and green:
        ...
        if risk > 0:
            ...
            reason = (
                f"Trend ↑ (EMA200), EMA20>EMA50, RSI14={rsi14:.1f}>55, "
                f"vela verde. ATR14={atr14:.1f}."
            )
            signals.append(
                US30Signal("BUY", round(price, 2), round(stop, 2), round(tp, 2), rr_target, reason)
            )
    ...
    if price < ema200 and ema20 < ema50 and rsi14 < 45 and red:
        ...
        if risk > 0:
            tp = price - rr_target * risk
            reason = (
                f"Trend ↓ (EMA200), EMA20<EMA50, RSI14={rsi14:.1f}<45, "
                f"vela roja. ATR14={atr14:.1f}."
            )
            signals.append(
                US30Signal("SELL", round(price, 2), round(stop, 2), round(tp, 2), rr_target, reason)
            )
