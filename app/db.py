...
def init_db(db_path: str):
    ...
    with sqlite3.connect(db_path) as con:
        con.execute(
            "CREATE TABLE IF NOT EXISTS sent_items (url TEXT PRIMARY KEY, first_sent_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);"
        )
        con.execute(
            "CREATE TABLE IF NOT EXISTS sent_signals (sig_hash TEXT PRIMARY KEY, payload TEXT, first_sent_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);"
        )
        con.commit()
...
def mark_signal(db_path: str, sig: dict):
    ...
    with connect(db_path) as con:
        con.execute(
            "INSERT OR IGNORE INTO sent_signals (sig_hash, payload) VALUES (?, ?);",
            (h, json.dumps(sig)),
        )
        con.commit()